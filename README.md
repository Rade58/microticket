...
MOZES DA UPOREDIS ONO STA SI TI SLAO SA ONIM STA STA SALJE `expreess-validator` LIBRARY, U SLUCAJU KADA JE NESTO INVALID (KAO ERROR DAT SALJE SE ARRAY SA OBJEKTIMA KOJI NA SEBI IMAJU `msg`; ZATIM IMA I INFO O IMENU INVALID FIELD-A, A TO JE PROPERTI `param` ,ZATIM SE SALJE I KOJA JE TA INVALID VALUE, U `value` PROPERTIJU)

DAKLE OPISANO JE, KOJI JE PROPERTI FAIL-OVAO VALIDATION, ZATIM PROVIDEED JE MESSAGE O TOME WHAT WENT WRONG AND HOW TO FIX IT
