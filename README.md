...
ISTE TESTOVE, MOZES PONOVITI I ZA MODELE, KOJI SU U `orders` MICROSERVICE-U

## EVO PRAVIM CONCURRENCY CONTROLL TEST I ZA MODEL IZ `orders` MICROSERVICE-A

- `mkdir orders/src/models/__test__`

- `touch orders/src/models/__test__/ticket.model.test.ts`
...
- `touch orders/src/models/__test__/order.model.test.ts`
...
MOGU DA RUNN-UJEM DVA TEST SUITE-A, KOJA SAM NAPRAVIO
...
# SAMO DA TI KAZEM DA JE POZIV `save`-A, TO STO INCEMENTIRA version

STO ZNACI DA AKO POZOVES save NA QUERIED DOKUMENTU, I BEZ DA SI TI ISTA KONKRETNO MENJA (ODNOSNO DA NISI NI POZVAO set), TO CE INCREMENTIRATI VERSION

# PRIMETIO SI DA GORNJI TESTOVI NISU NEKA SIMULACIJA ZA SLANJE EVENT-OVA OUT OF ORDER, JER NAS ZANIMA TAJ CONCURRENY PROBLEM

E PA, I SVAKI PUBLISHED EVENT BI NA SEBI TREBALO DA IMA I `version` FIELD

TIME CU SE POZABAVITI U SLEDECEM BRANCH-U, KAKO BI U INTERFACE-OVIMA, KOJI TYPE-UJU RAZLICITE EVENTS, USTVARI UVRSTIM I `version` FIELD
