...
COOKIE MOZES DA PROCITAS I KADA NADJES NETWORK REQUEST U BROWSERU, A TO JE UNDER `Network -> <tvoj reques> -> 'Headers'`, PA PRONADJI `set-cookie` (ZA SVAKI FOLLOW UP REQUEST COOKIE HEADER CE BITI INCLUDED IN REQUEST (I VIDJACES `cookie` HEADER SA POMENUTOM VREDNOSCU)) (RELOAD-UJ PAGE, PA PROVERI COOKIE (DA ZAISTA SADA JE TAMO `cookie` HEADER))

# MEDJUTIM AUTOR WORKSHOPA VOLI axios VISE NEGO `window.fetch` TAKO DA CU INSTLAIRATI `axios` I ONDA CU NJEGA USTVARI UPOTREBITI ZA SLANJE NETWORK REQUEST-OVA

- `cd client`

- `yarn add axios`

## SADA CU DA U CODEBASE-U `fetch` ZAAMENIM `axios`-OM
